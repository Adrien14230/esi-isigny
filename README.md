...
-- Photos (legacy — autogenerated thumbnails par agent-08)
...
create table gallery_approved (
  ...
  title text, email text, path text not null, thumb_path text,
  approved_at timestamptz default now(),
  approved_by text
);
create index gallery_approved_category_idx on gallery_approved (category, approved_at desc);

-- Bucket storage 'gallery' (private, accès via signed URLs)
-- Créer dans Supabase Studio > Storage > New bucket "gallery" (private)
